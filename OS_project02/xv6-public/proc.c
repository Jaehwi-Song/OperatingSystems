...
found:
  ...
  // initialize new process's qlevel, tick, prioirity with 0
  p->qlevel = 0;
  p->ticks = 0;
  p->priority = 0;
  ...
  for(;;){
	...
	if (p == 0 || p->state != RUNNABLE)
        ...
        for (p2 = ptable.proc; p2 < &ptable.proc[NPROC]; p2++) {
          if (c->monopolized == 1)
	    break;
	  if (p2 == p || p2 == 0 || p2->state != RUNNABLE)
            continue;
        ...
        for (p3 = ptable.proc; p3 < &ptable.proc[NPROC]; p3++) {
          if (c->monopolized == 1)
	    break;
	  if (p3 == p || p3 == 0 || p3->state != RUNNABLE)
            continue;
        ...
        // else, sheduling process in lower queue first.
        else
          p = lowest_queue_proc;
...
// return level of Queue, which current process is belonging to.
int 
getlev(void)
{
